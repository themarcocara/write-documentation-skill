...
#
# In the Gem, don't invoke this via the __main__/CLI path -- there's no filesystem to
# point it at. Paste the whole script into a code-execution call, then call
# compute_metrics(text) directly with the text as a Python string. For a before/after
# comparison (the write-docs final score check), call compute_metrics() on both strings
# and build the same {"before": ..., "after": ...} shape the --diff CLI mode below
# produces. Entry point: compute_metrics(text: str) -> dict.
#
# Ported verbatim (stdlib-only, no adaptation needed to the scoring logic itself) from
# the original .claude/skills/readability/scripts/readability_score.py.
